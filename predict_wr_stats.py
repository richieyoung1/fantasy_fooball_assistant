...
def load_and_merge(year):
    stats = pd.read_csv(f"wr_{year}_stats.csv")
    features = pd.read_csv(f"wr_{year}_features.csv")
...
def main():
    for year in range(START_YEAR, END_YEAR):
        print(f"\n🧪 WR: Training on {year}, predicting {year + 1}")
        try:
            ...
            print(f"❌ Skipping WR {year}->{year+1}: {e}")
            continue

        results = pd.DataFrame({"player": players})

        for target in TARGET_COLUMNS:
            print(f"🚀 Predicting WR stat: {target}")
        ...
        results.to_csv(f"wr_predictions_{year+1}.csv", index=False)
        print(f"✅ Saved WR predictions to wr_predictions_{year+1}.csv")
